...
void main(void)
{
    ...
    //initialize super special message
    for(i=0; i<RF_24G_PAYLOADSIZE; i++){
        RF_24G_Buffer[i] = RF_24G_PAYLOADSIZE-i;
    }
    puts("Server.\r\n");
...
{
    int timer=0;
    for(timer=0; timer<OPEN_COUNT; timer++){
        puts("Opening   ");
        putBuffer();
    }
}
